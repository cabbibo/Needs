...
function Loader(  params ){
  ...
  Loader.prototype = {
    ...
    addFailureDialog: function(){
      ...
      this.failureAudio = document.createElement( 'div' );
      this.failureAudio.innerHTML = '<iframe width="90%" height="450" scrolling="no" frameborder="no" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/playlists/42891797&amp;auto_play=false&amp;hide_related=false&amp;show_comments=true&amp;show_user=true&amp;show_reposts=false&amp;visual=true"></iframe>'

      this.failureLink = document.createElement( 'a' );
      this.failureLink.href = failureLink;
      this.failureLink.id = 'failureLink';

      this.itsOK = document.createElement( 'div' );
      this.itsOK.innerHTML = "<br/><br/>Don't worry though, you can still hear the immeccable noises made by the space beast that is JJ-Verne:"
   
      this.itsOK.style.width = "90%";
      this.itsOK.style.paddingLeft = "5%";

      this.failureDialog.appendChild( failureReasons );
      this.failureDialog.appendChild( this.failureList );
      this.failureDialog.appendChild( this.itsOK );
      this.failureDialog.appendChild( this.failureAudio );
